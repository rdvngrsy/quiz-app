...
const QuestionsCard = (props: Props) => {
  ...
  const approvedChoice = (e: any) => {
    ...
    const checkAnswer = e.currentTarget.value == props.questionsData[props.count]?.correct_answer;
    if (checkAnswer) {
  ...
  useEffect(() => {
    if (props.questionsData[props.count]) {
      ...
      dispatch(setCorrectAnswers(props.questionsData[props.count].correct_answer))
    }
  ...
  return (
    <div>
      <div className="bg-white max-w-[1000px] w-full flex flex-col items-center p-8 rounded-3xl space-y-4 relative border-4 border-black shadow-[5px_5px_0px_0px_rgba(0,0,0)]">
        <div className={`absolute mt-[-115px] bg-white w-40 h-40 font-bold text-[45px] flex justify-center items-center rounded-full border-4 border-black shadow-[5px_5px_0px_0px_rgba(0,0,0)] ${timer<10 ? "bg-red-600 text-white heartbeat" : "bg-white text-black"}`}>
          {timer}
        </div>
        ...
        {props.questionsData[props.count]?.answers?.map((answer, index) => (
          <button
            ...
            ${timer > 20 ? 'opacity-50 cursor-not-allowed' : ''} transition ease-in duration-200`}
